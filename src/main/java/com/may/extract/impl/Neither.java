 ...
 * 两者都没有
 */
public class Neither extends AbstarctExtractStrategy {

    public Neither(String str) {
        super(str);
    }
    
    private static Pattern PATTERN = Pattern.compile("借款服务类型：(\\S+)合同编号：(\\d+)借款人姓名：(\\S+)身份证号：(\\d{17}[0-9|x|X])居住地址：(\\S+)联系电话：(\\S+)产品系列代码：(\\S+)产品代码：(\\S+)借款金额：(\\S+)借款期限：(\\S+)年化借款利率：(\\S+)年化客户服务费率：(\\S+)还款方式：(\\S+)还款日：(\\S+)到期还款金额：(\\S+)借款用途：(\\S+)还款账户：账户名：(\\S+)银行名称：(\\S+)银行账号：(\\d+)");
    ...
    private Supplier<Entity> operator = () -> {
        ...
        if (matcher.find()){
            ...
            entity.set("ec_pay_type",matcher.group(13));
            entity.set("ec_pay_time",matcher.group(14));
            entity.set("ec_pay_amount",matcher.group(15));
            entity.set("ec_loan_use",matcher.group(16));
            entity.set("ec_pay_name",matcher.group(17));
            entity.set("ec_pay_bank",matcher.group(18));
            entity.set("ec_pay_bank_num",matcher.group(19));
        }
        return entity;
    };
